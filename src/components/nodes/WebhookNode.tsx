...
import { Webhook, Trash2, Link, ArrowRight } from 'lucide-react';
...
const WebhookNode: React.FC<WebhookNodeProps> = ({ id, ports, portRefs, onPortMouseEnter, onPortMouseLeave, onPortMouseUp, onPortClick, onPortMouseDown, x, y, canDelete, onDelete, onDoubleClick, onNodeMouseDown }) => {
  ...
  >
    {canDelete && (
      <button
        className="absolute top-1 right-1 p-1 rounded-md bg-yellow-50 hover:bg-yellow-100 text-yellow-500 hover:text-yellow-700 transition-all duration-200 node-trash"
    ...
    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ pointerEvents: 'none' }}>
      <Webhook className="w-7 h-7 text-yellow-600" />
    </div>
