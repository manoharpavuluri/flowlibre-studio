...
import { BookOpen, ArrowRight, Link, Trash2, FileText } from 'lucide-react';
...
interface DocumentIntelligenceParserNodeProps {
  id: string;
  ports: NodePort[];
...
const DocumentIntelligenceParserNode: React.FC<DocumentIntelligenceParserNodeProps & { iconOverride?: React.ComponentType<any> }> = ({
  id,
  ports,
  portRefs,
  onPortMouseEnter,
  ...
  >
    ...
    {showIcon && (
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ pointerEvents: 'none' }}>
        {iconOverride ? (
          React.createElement(iconOverride, { className: 'w-7 h-7' })
        ) : (
...
export default DocumentIntelligenceParserNode; 