...
import { Search, Trash2, Link, ArrowRight } from 'lucide-react';
...
interface FuzzyLookupNodeProps {
  id: string;
  ports: NodePort[];
...
const FuzzyLookupNode: React.FC<FuzzyLookupNodeProps> = ({ id, ports, portRefs, onPortMouseEnter, onPortMouseLeave, onPortMouseUp, onPortClick, onPortMouseDown, x, y, canDelete, onDelete, onDoubleClick, onNodeMouseDown }) => {
  ...
  >
    {canDelete && (
      <button
        className="absolute top-1 right-1 p-1 rounded-md bg-green-50 hover:bg-green-100 text-green-500 hover:text-green-700 transition-all duration-200 node-trash"
...
export default FuzzyLookupNode; 