...
import { CheckSquare, Trash2, Link, ArrowRight } from 'lucide-react';
...
interface ReconciliationNodeProps {
  id: string;
  ports: NodePort[];
...
const ReconciliationNode: React.FC<ReconciliationNodeProps> = ({ id, ports, portRefs, onPortMouseEnter, onPortMouseLeave, onPortMouseUp, onPortClick, onPortMouseDown, x, y, canDelete, onDelete, onDoubleClick, onNodeMouseDown }) => {
  ...
  >
    {canDelete && (
      <button
        className="absolute top-1 right-1 p-1 rounded-md bg-sky-50 hover:bg-sky-100 text-sky-500 hover:text-sky-700 transition-all duration-200 node-trash"
    ...
    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ pointerEvents: 'none' }}>
      <CheckSquare className="w-7 h-7 text-sky-600" />
    </div>
...
export default ReconciliationNode; 